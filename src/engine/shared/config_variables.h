...
//chillerbot FC
MACRO_CONFIG_STR(ClRestartCommand, cl_restart_command, 512, "echo \"WARNING the server crashed and no crash command was set. Set one with 'cl_restart_command'\" > crash_info.txt", CFGFLAG_SAVE | CFGFLAG_CLIENT, "command will be executed in terminal if connection to server gets lost")
MACRO_CONFIG_STR(ClConnectIP, cl_connect_ip, 32, "none", CFGFLAG_SAVE | CFGFLAG_CLIENT, "gets initializied as 0 in collection")

MACRO_CONFIG_STR(PlayerName, player_name, 16, "chillerbot", CFGFLAG_SAVE|CFGFLAG_CLIENT, "Name of the player")
