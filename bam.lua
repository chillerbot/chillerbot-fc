...
function build(settings)
	...
	fake_client = Compile(settings, Collect("src/fakeclient/*.cpp"))
	pen_client = Compile(settings, Collect("src/penclient/*.cpp"))
	vanilla_bot = Compile(settings, Collect("src/vanillabot/*.cpp"))
	...
	fakeclient_exe = Link(client_settings, "chillerbot-FC-0003", fake_client,
		engine, zlib, game_shared)
	penclient_exe = Link(client_settings, "PENTESTER_nogui", pen_client,
		engine, zlib, game_shared)
	vanillabot_exe = Link(client_settings, "chillerbot-FC-vanilla", vanilla_bot,
		engine, zlib, game_shared)
	...
	f = PseudoTarget("fakeclient".."_"..settings.config_name, fakeclient_exe)
	p = PseudoTarget("penclient".."_"..settings.config_name, penclient_exe)
	b = PseudoTarget("vanillabot".."_"..settings.config_name, vanillabot_exe)

	all = PseudoTarget(settings.config_name, c, s, v, m, t, f, p, b)
	return all
end
